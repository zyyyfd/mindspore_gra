...
from mind_utils import get_train_val_test, get_train_val_test_gcn
...
class Dataset():
    ...
    def __init__(self, root, name, setting='nettack', seed=None, require_mask=False):
        ...
        assert self.name in ['cora', 'citeseer', 'cora_ml', 'polblogs', 'pubmed', 'aids', 'enzyme', 'usair', 'brazil',                             'blogcatalog','europe', 'chameleon', 'squirrel', 'ogb_arxiv'],             'Currently only support cora, citeseer, cora_ml, polblogs, pubmed, aids, enzyme, usair, brazil, europe'
    ...
    def load_ogb_arxiv(self):
        ...
        # all are numpy array
        x = ms.load(osp.join(data_folder, 'x.pt'))
        y = ms.load(osp.join(data_folder, 'y.pt'))
        adj = ms.load(osp.join(data_folder, 'adj.pt'))
