...
from mind_topology_attack import PGDAttack
...
def init_env(args):
    ...
    if args.context_mode not in ["graph", "pynative"]:
        raise ValueError(f"Invalid context_mode: {args.context_mode}, "
                         f"should be in ['graph', 'pynative']")
    context_mode = ms.GRAPH_MODE if args.context_mode == "graph" else ms.PYNATIVE_MODE
    ...
    # 设置运行时使用的卡
    if hasattr(args, "device_id") and isinstance(args.device_id, int):
        ms.set_context(device_id=args.device_id)
    ...
    if args.device_num > 1:
        ...
        if args.device_num != group_size:
            raise ValueError(f"the setting device_num: {args.device_num} not equal to the real group_size: {group_size}")
        args.rank_id = get_rank()
        ms.set_auto_parallel_context(parallel_mode=ms.ParallelMode.DATA_PARALLEL, gradients_mean=True)
        if hasattr(args, "all_reduce_fusion_config"):
...
parser.add_argument('--context_mode', type=str, default='graph')
...
init_adj = ms.Tensor(init_adj.todense())
...
if args.arch == 'gat':
    victim_model = GAT(nfeat=features.shape[1], nclass=labels.max().item() + 1,nhid=16,  nlayer=args.nlayers,
                       dropout=0.5, alpha=0.1, nheads=5)
    ...
    embedding = embedding_gat(nfeat=features.shape[1], nclass=labels.max().item() + 1,  nlayer=args.nlayers,
                              dropout=0.5, alpha=0.1, nheads=5)
    embedding.load_state_dict(transfer_state_dict(
        victim_model.state_dict(), embedding.state_dict()))
    ...
    for i in tqdm(range(n)):
        for j in range(n):
            if labels[i] == labels[j]:
                ...
                label_adj[i][j] = 0
    label_adj = ms.Tensor(label_adj).numpy()
...
def gaussian_reparameterize_attack(arg):
    ...
    gaussian_model = gaussian_parameterized.PGDAttack(
        H_A=H_A2, Y_A=Y_A, model=victim_model, embedding=embedding, nnodes=adj.shape[0], loss_type="CE")
...
def gcn_reparameterize_attack(arg):
    ...
    gcn_model = gcn_parameterized.PGDAttack(H_A=H_A2, Y_A=Y_A, features=features, model=victim_model,
                                            embedding=embedding, nnodes=adj.shape[0], loss_type="CE")
...
get_ipython().system('jupyter nbconvert --to script mind_main.ipynb')
